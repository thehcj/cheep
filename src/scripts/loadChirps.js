    ...
    fetch(`/fetch_chirps.php?offset=${offset}`)
      ...
      .then((chirps) => {
        ...
        chirps.forEach((chirp) => {
          ...
          userPic.src = chirp.profilePic
            ? chirp.profilePic
            : "/src/images/users/guest/user.svg";
          ...
          replyButton.onclick = () =>
            (location.href = `/chirp/?id=${chirp.id}`);
          ...
          rechirpButton.onclick = () =>
            updateChirpInteraction(chirp.id, "rechirp", rechirpButton);
          ...
          likeButton.onclick = () =>
            updateChirpInteraction(chirp.id, "like", likeButton);
      ...
      .catch((error) => {
        ...
        if (error && error.status === 401) {
          ...
          chirpsContainer.innerHTML =
            '<div class="signInPrompt"><p>Please sign in with BlueSky to view your timeline.</p><a href="/signin/" class="button followButton">Sign in with BlueSky</a></div>';
        }
      })
      .finally(() => {
...
function updateChirpInteraction(chirpId, action, button) {
  console.log(
    `Updating chirp interaction for chirp ID: ${chirpId}, action: ${action}`,
  );
  fetch(`/interact_chirp.php`, {
    ...
    .then((data) => {
      ...
      if (data.success) {
        ...
        if (action === "like") {
          button.querySelector("img").src = data.like
            ? "/src/images/icons/liked.svg"
          ...
          button.querySelector("img").src = data.rechirp
            ? "/src/images/icons/rechirped.svg"
